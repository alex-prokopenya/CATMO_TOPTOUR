<%@ WebHandler Language="C#" Class="TopTourMiddleOffice.json_handler" %>
using System;
...
namespace TopTourMiddleOffice
{
    ...
    public class json_handler : JsonRpcHandler
    {
        ...
        public json_handler()
        {
            ...
            JsonRpcDispatcherFactory.Current = s => new TopTourMiddleOffice.JsonRpcDispatcher(s);
        }
	    ...
	    [JsonRpcHelp("{\"jsonrpc\":\"2.0\",\"method\":\"create_new_dogovor\",\"params\":[[1,2,3,4], {\"email\":\"user@test.com\",\"phone\":\"+375 29 111 22 33\"}],\"id\":0,\"packettype\":0}")]
        public  object create_new_dogovor(JsonArray book_ids, JsonObject user_info, string info, string login, int packettype = 0)
        {
            ...
            //создаем заказ
            return middle_office.CreateNewDogovor(services_, userInfo, info,  login, type);
        }

        //++++++
        //аннулируем заказ, либо набор услуг из заказа
        [JsonRpcMethod("annulate_dogovor")]
	    [JsonRpcHelp("{\"jsonrpc\":\"2.0\",\"method\":\"annulate_dogovor\",\"params\":[\"tt@rr.tt\",'TR4534',[]],\"id\":0}")]
        public object annulate_dogovor(string user_mail, string dogovor_code, params object[] args)
        {
            //проверяем номер путевки на валидность
            if (!Validator.CheckDogovorCode(dogovor_code)) throw new CatmoException("Invalid dogovor_code", ErrorCodes.InvalidDogovorCode);
            ...
            return middle_office.AnnulateDogovor(user_mail, dogovor_code, service_keys);
        }
        ...
        public object get_dogovor_info(string user_mail, string dogovor_code)
	    {
            if (!Validator.CheckDogovorCode(dogovor_code)) throw new CatmoException("Invalid dogovor_code", ErrorCodes.InvalidDogovorCode);

            return middle_office.GetDogovorInfo(user_mail, dogovor_code);
        }
        ...
        public object get_dogovor_messages(string user_mail, string dogovor_code, string min_date_time)
        {
            if (!Validator.CheckDogovorCode(dogovor_code)) throw new CatmoException("Invalid dogovor_code", ErrorCodes.InvalidDogovorCode);
            ...
            try
            {
                minDate = DateTime.ParseExact(min_date_time, "dd.MM.yyyy HH:mm:ss", null); 
            }
            catch (Exception)
            {
            ...
            return middle_office.GetDogovorMessages(user_mail, dogovor_code, minDate);
        }
        ...
        public object send_message(string user_mail, string dogovor_code, string text)
        {
            if (!Validator.CheckDogovorCode(dogovor_code)) throw new CatmoException("Invalid dogovor_code", ErrorCodes.InvalidDogovorCode);
            ...
            return middle_office.SendMessage(user_mail, dogovor_code, text);
        }
        ...
        public object add_service(string dogovor_code, int book_id, int packettype = 0)
        {
            ...
            return middle_office.AddService(dogovor_code, book_id, type);
        }
